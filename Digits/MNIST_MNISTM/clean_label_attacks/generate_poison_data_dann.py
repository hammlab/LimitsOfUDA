...
from utils import eval_accuracy_main_cdan, eval_accuracy_dc, clip_eta
...
domain_classifier = [mnist2mnistm_domain_predictor_CDAN(shared_1[i], NUM_CLASSES_DC, 500)  for i in range(NUM_MODELS)]
...
def train_step_da_1(main_data, main_labels, domain_data, opposite_domain_labels):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        shared_domain = [shared_1[i](domain_data, training=True) for i in range(NUM_MODELS)]
        domain_logits = [domain_classifier[i](shared_domain[i], training=True) for i in range(NUM_MODELS)]
        domain_loss = [ce_loss(opposite_domain_labels, domain_logits[i]) for i in range(NUM_MODELS)]
...
def train_step_da_2(domain_data, true_domain_labels):
    ...
    with tf.GradientTape(persistent=True) as tape:
        shared_domain = [shared_1[i](domain_data, training=True) for i in range(NUM_MODELS)]
        domain_logits = [domain_classifier[i](shared_domain[i], training=True) for i in range(NUM_MODELS)]
        domain_loss = [ce_loss(true_domain_labels, domain_logits[i]) for i in range(NUM_MODELS)]
        
        loss = [domain_loss[i] for i in range(NUM_MODELS)]
        
    gradients_domain_classifier = [tape.gradient(loss[i], domain_classifier[i].trainable_variables) for i in range(NUM_MODELS)]
...
def train_step_poison(base_data, target_data, poison_data):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        representation_1_u = [shared_1[i](u, training=False) for i in range(NUM_MODELS)]
        representation_1_target = [shared_1[i](target_data, training=False) for i in range(NUM_MODELS)]
...
for epoch in range(EPOCHS):
    ...
    if epoch % 20 == 0:
        #tf.print(l1/len(x_poison), l2/len(x_poison))
        print(x_train_mnist.shape, "BASE_DOMAIN:", BASE_DOMAIN)
        print(METHOD + " training: MNIST->MNIST_M:", epoch)
        print([eval_accuracy_main_cdan(x_target_point, y_target_point_incorrect_label, shared_1[i], main_classifier[i])  for i in range(NUM_MODELS)])
        print([eval_accuracy_main_cdan(x_target_point, y_target_point, shared_1[i], main_classifier[i]) for i in range(NUM_MODELS)])
        print([eval_accuracy_main_cdan(x_test_mnistm, y_test_mnist, shared_1[i], main_classifier[i]) for i in range(NUM_MODELS)])
        print([eval_accuracy_dc(np.concatenate([x_test_mnistm, x_test_mnist]), np.concatenate([keras.utils.to_categorical(np.ones(len(x_test_mnistm)), NUM_CLASSES_DC), keras.utils.to_categorical(np.zeros(len(x_test_mnist)), NUM_CLASSES_DC)]), shared_1[i], domain_classifier[i]) for i in range(NUM_MODELS)])
