...
parser.add_argument('--TYPE', type=str, default="CLEAN", help='CLEAN or POISON')
...
for epoch in range(EPOCHS):
    ...
    if epoch % 20 == 0:  
        ...
        discriminator_acc = np.array([eval_accuracy_dc(np.concatenate([x_test_mnistm, x_test_mnist]), np.concatenate([keras.utils.to_categorical(np.ones(len(x_test_mnistm)), NUM_CLASSES_DC), keras.utils.to_categorical(np.zeros(len(x_test_mnist)), NUM_CLASSES_DC)]), shared_1[i], domain_classifier[i]) for i in range(NUM_MODELS)])
