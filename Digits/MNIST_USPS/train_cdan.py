...
parser.add_argument('--ALPHA', type=str, default="0.8", help='ALPHA')
...
for epoch in range(EPOCHS):
    ...
    if epoch%20 == 0:
        ...
        discriminator_acc = np.array([eval_accuracy_dc_cdan_random(np.concatenate([usps_x_test, mnist_x_test]), 
                                   np.concatenate([keras.utils.to_categorical(np.ones(len(usps_x_test)), NUM_CLASSES_DC), 
                                                   keras.utils.to_categorical(np.zeros(len(mnist_x_test)), NUM_CLASSES_DC)]),         
        ...
        print("CDAN MNIST->USPS:", epoch, TYPE, ALPHA if TYPE == "POISON" else "",
             target_acc,
             source_acc,
             discriminator_acc)
        if TYPE == "POISON":
