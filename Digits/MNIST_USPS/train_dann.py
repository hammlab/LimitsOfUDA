...
import numpy as np
import keras
from utils import eval_accuracy_main, eval_accuracy_dc, load_usps
from models import mnist2mnistm_shared_CDAN, mnist2mnistm_softmax_CDAN, mnist2mnistm_domain_predictor_CDAN
import argparse

parser = argparse.ArgumentParser(description='Training', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument('--TYPE', type=str, default="POISON", help='CLEAN or POISON')
parser.add_argument('--ALPHA', type=str, default="0.0", help='ALPHA')
...
def train_step_da_1(main_data, main_labels, domain_data, opposite_domain_labels):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        shared_domain = [shared[i](domain_data, training=True) for i in range(NUM_MODELS)]
...
ckpt = [tf.train.Checkpoint(shared_1 = shared[i],
...
if TYPE == "POISON":
    ...
    if True:
        ...
        for i in range(len(poison_indices)):
            ...
            poisoned_LF_indices.append(class_indices[label][indx])
    ...
    if ALPHA == 0:
        mnist_x_train = np.array(np.delete(mnist_x_train, poisoned_LF_indices, axis = 0))
        mnist_y_train = np.array(np.delete(mnist_y_train, poisoned_LF_indices, axis = 0))
...
for epoch in range(EPOCHS):
    ...
    if epoch%20 == 0:
        ...
        discriminator_acc = np.array([eval_accuracy_dc(np.concatenate([usps_x_test, mnist_x_test]), np.concatenate([keras.utils.to_categorical(np.ones(len(usps_x_test)), NUM_CLASSES_DC), keras.utils.to_categorical(np.zeros(len(mnist_x_test)), NUM_CLASSES_DC)]), shared[i], domain_classifier[i]) for i in range(NUM_MODELS)])
        print("DANN MNIST->MNIST_m:", epoch, TYPE, ALPHA if TYPE == "POISON" else "",
             target_acc,
             source_acc,
             discriminator_acc)
        if TYPE == "POISON":
