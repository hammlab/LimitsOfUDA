...
import numpy as np
import keras
from utils import eval_accuracy_main, load_svhn, load_mnist, eval_accuracy_dc
...
domain_classifier = [svhn2mnist_domain_predictor_CDAN(shared[i], NUM_CLASSES_DC, 512) for i in range(NUM_MODELS)]
...
for epoch in range(EPOCHS):
    ...
    if epoch%20 == 0:
        ...
        discriminator_acc = np.array([eval_accuracy_dc(np.concatenate([svhn_x_test, mnist_x_test]), np.concatenate([keras.utils.to_categorical(np.zeros(len(svhn_x_test)), NUM_CLASSES_DC), keras.utils.to_categorical(np.ones(len(mnist_x_test)), NUM_CLASSES_DC)]), shared[i], domain_classifier[i]) for i in range(NUM_MODELS)])
        print("DANN SVHN->MNIST:", epoch, TYPE, ALPHA if TYPE == "POISON" else "",
             target_acc,
             source_acc,
             discriminator_acc)
        if TYPE == "POISON":
