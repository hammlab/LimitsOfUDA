...
for epoch in range(EPOCHS):
    ...
    if epoch%20 == 0:
        ...
        discriminator_acc = np.array([eval_accuracy_dc(np.concatenate([usps_x_test, mnist_x_test]), np.concatenate([keras.utils.to_categorical(np.zeros(len(usps_x_test)), NUM_CLASSES_DC), keras.utils.to_categorical(np.ones(len(mnist_x_test)), NUM_CLASSES_DC)]), shared[i], domain_classifier[i]) for i in range(NUM_MODELS)])
        print("DANN USPS->MNIST:", epoch, TYPE, ALPHA if TYPE == "POISON" else "",
             target_acc,
             source_acc,
             discriminator_acc)
        if TYPE == "POISON":
