...
print(mnist_x_train.shape, usps_x_train.shape, mnist_y_train.shape, usps_y_train.shape, np.min(usps_x_train), np.max(usps_x_train))
print(mnist_x_test.shape, usps_x_test.shape)
...
for epoch in range(EPOCHS):
    ...
    if epoch%20 == 0:
        ...
        source_acc = np.array([eval_accuracy_main(usps_x_test, usps_y_test, shared[i], main_classifier_1[i]) for i in range(NUM_MODELS)])
        print("MCD USPS->MNIST:", epoch, TYPE, ALPHA if TYPE == "POISON" else "",
             target_acc,
             source_acc)
        if TYPE == "POISON":
